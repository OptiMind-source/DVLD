...
namespace DVLD_System.People
{
    public partial class frmListPeople : Form
    {
        ...
        private void _RefreshPeoplList()
        {
            ...
            _dtPeople.Columns.Add(GenderColumn);
            GenderColumn.SetOrdinal(6); // Move to desired position

            // Add gender info by matching PersonID
            foreach (DataRow personRow in _dtPeople.Rows)
            {
                int personId = Convert.ToInt32(personRow["PersonID"]);

                DataRow[] match = _dtAllPeople.Select("PersonID = " + personId);
                if (match.Length > 0)
                {
                    int genderValue = Convert.ToInt32(match[0]["Gender"]);
                    personRow["Gender"] = (genderValue == 0) ? "Male" : "Female";
                }
            }
